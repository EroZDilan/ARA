...
class App {
  ...
  constructor(
    ...
    const prefersReducedMotion =
      typeof window !== 'undefined' && window.matchMedia
        ...
        : false;
    this.scroll = { ease: prefersReducedMotion ? 1 : scrollEase, current: 0, target: 0, last: 0 };
    ...
    // Pause the render loop while the gallery is scrolled out of view.
  ...
  onWheel(e: Event) {
    ...
    // Only react to genuinely horizontal gestures (trackpad swipe). A normal
    // vertical mouse wheel should keep scrolling the page, not this gallery —
    // driving both from the same deltaY caused the page and the gallery to
    // move at once, which read as jitter.
  ...
  addEventListeners() {
    ...
    // Wheel/drag-start are scoped to the gallery's own container, not window,
    // so this component doesn't hijack scroll/click input elsewhere on the page.
    this.container.addEventListener('mousewheel', this.boundOnWheel);
    this.container.addEventListener('wheel', this.boundOnWheel);
    this.container.addEventListener('mousedown', this.boundOnTouchDown);
    this.container.addEventListener('touchstart', this.boundOnTouchDown, { passive: true });
    // Drag continuation stays on window so it doesn't break if the pointer
    // leaves the container mid-drag; onTouchMove/onTouchUp no-op when !isDown.
